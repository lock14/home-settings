...
if [ -f "$SCRIPT_DIR/.mise.toml" ]; then
    if grep -q 'java = "lts"' "$SCRIPT_DIR/.mise.toml" && grep -q 'node = "lts"' "$SCRIPT_DIR/.mise.toml" && grep -q 'go = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'maven = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'python = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'neovim = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'eza = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'bat = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'glow = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'tree-sitter = "latest"' "$SCRIPT_DIR/.mise.toml" && grep -q 'go.set_gobin = false' "$SCRIPT_DIR/.mise.toml" && grep -q 'go.set_gopath = false' "$SCRIPT_DIR/.mise.toml"; then
