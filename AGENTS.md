...
| **Namespaces & Modules** | `violet` | `#6C71C4` | Namespaces, modules, package qualifiers (`std::`, `core::telemetry::`), `@module` |
...
1. **3-Tier Ergonomic Architecture**: All syntax highlighting across Neovim, `bat`, and shell environments strictly follows the 3-Tier cognitive hierarchy:
   ...
   - **Tier 3: Values & Directives (5–10% screen area)**: Cyan (`#2AA198`), Magenta (`#D33682`), Orange (`#CB4B16`). Highlights payload data: Cyan for strings and unbroken escape sequences/format specifiers (`\n`, `%s`); Magenta for numeric literals, booleans (`true`, `false`), named `ALL_CAPS` constants/enum members (`EXIT_FAILURE`, `MAX_BUFFER_SIZE`, `LOG_ERROR`, `NodeState::Initializing`), and sentinels (`std::nullopt`); Orange for preprocessor directives (`#define`, `#include`, `#ifdef`).
