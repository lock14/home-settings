...
4. **Standalone Subsystem Independence (The Dependency Trap)**:
   ...
   - All toolchain grammars, tree-sitter queries, and shell configurations in this repository must be **completely self-contained**. Distributing standalone grammars (`C.sublime-syntax`, `C++.sublime-syntax`, `Diff.sublime-syntax`, `Go.sublime-syntax`, `Java.sublime-syntax`, `Python.sublime-syntax`) in `syntaxes/` ensures deterministic behavior without external dependency traps.
...
8. **Temporal Grammatical Divergence (The LTS Currency Imperative)**:
   ...
   - Workstations provisioning actively supported LTS language runtimes (Java 17/21/25, C++20/23, Go 1.22+) must decouple themselves from upstream grammar stagnation by maintaining version-controlled, standalone grammars and Tree-sitter query overrides in `syntaxes/` and `after/queries/`.
...
6. **Neovim Lua**: Uses `maxmx03/solarized.nvim` with `variant = "spring"` matching `bat` 1:1, integrated with Native Neovim 0.11+ LSP (`vim.lsp.config`, `LspAttach`) and Tree-sitter queries in `after/queries/` (including `c/highlights.scm`, `cpp/highlights.scm`, `diff/highlights.scm`, `go/highlights.scm`, `java/highlights.scm`, and `python/highlights.scm` capturing `package` in Green, `import` in Orange, `sizeof` type arguments as `@type`, factory functions as Blue, records/guards as Green, annotations/decorators as Orange, and method initializers as Blue).
