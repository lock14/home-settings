...
1. **`bat` (Syntect CLI File Viewer)**:
   ...
   - Standalone modern Sublime grammars in `syntaxes/` (`C`, `C++`, `Diff`, `Go`, `Java`, `Python`, `Rust`, `Bash`, `SQL`, `Terraform`, `Markdown`, `TypeScript`, `XML`, `HTML`, `JSON`, `TOML`, `CSS`).
...
2. **Neovim (Native LSP & Tree-sitter Editor)**:
   ...
   - Integrated with Native Neovim 0.11+ LSP (`vim.lsp.config`, `LspAttach`) and Tree-sitter query overrides in `after/queries/` (`c`, `cpp`, `diff`, `go`, `java`, `python`, `rust`, `bash`, `sql`, `terraform`, `markdown`, `markdown_inline`, `javascript`, `typescript`, `xml`, `toml`, `css`) and base query supersedure in `queries/` (`html`, `html_tags`, `xml`, `css`).
