...
5. **CLI Syntax Highlighting (`zsh-syntax-highlighting` / `ZLE`)**: Explicitly configured in `.zshrc-addendum` using 24-bit TrueColor Solarized Dark with restrained, non-distracting syntax highlighting (`commands` green `#859900`, `strings` cyan `#2AA198`, `functions`/`paths` blue `#268BD2`, `errors` red `#DC322F`, `comments`/`suggestions` base01 `#586E75`, `selection` base02 `#073642`, with options, parameters, assignments, and operators kept calm in neutral foreground base0 `#839496`).
