...
| **Bat Syntax Packages** | `syntaxes/*.sublime-syntax` (C, C++, Diff, Go, Java, Python, Rust, Bash, SQL, Terraform) | `${XDG_CONFIG_HOME:-$HOME/.config}/bat/syntaxes/` | `tests/test-dotfiles.sh` |
...
4. **Standalone Subsystem Independence (The Dependency Trap)**:
   ...
   - All toolchain grammars, tree-sitter queries, and shell configurations in this repository must be **completely self-contained**. Distributing standalone grammars (`C.sublime-syntax`, `C++.sublime-syntax`, `Diff.sublime-syntax`, `Go.sublime-syntax`, `Java.sublime-syntax`, `Python.sublime-syntax`, `Rust.sublime-syntax`, `Bash.sublime-syntax`, `SQL.sublime-syntax`, `Terraform.sublime-syntax`) in `syntaxes/` ensures deterministic behavior without external dependency traps.
...
22. **Declarative Scope Symmetry & Traversal Boundaries (Scope Keyword vs Member Qualifier)**:
   - In declarative domain-specific languages (such as HCL and Terraform), identifiers prefixed before a dot fulfill divergent architectural roles: engine-provided scope roots (`var.`, `local.`, `terraform.`, `data.`, `module.`, `count.`, `each.`) declare boundary scope namespaces, whereas user-defined resource blocks (`aws_s3_bucket.telemetry_lake.arn`) represent instanced entities and attributes.
   - Upstream parsers frequently conflate all pre-dot identifiers as built-in variables or constants (coloring `aws_s3_bucket` Magenta).
   - Scope roots must maintain declarative symmetry with top-level block definitions (`variable`, `locals`, `terraform`) as reserved keywords in **Solarized Green (`#859900`)**, while user-defined resource references and property accessors remain in calm **Base0 Grey (`#839496`)**.
23. **Interpolation Delimiter Boundary Isolation (Sigil Collision Prevention)**:
...
6. **Neovim Lua**: Uses `maxmx03/solarized.nvim` with `variant = "spring"` matching `bat` 1:1, integrated with Native Neovim 0.11+ LSP (`vim.lsp.config`, `LspAttach`) and Tree-sitter queries in `after/queries/` (including `c/highlights.scm`, `cpp/highlights.scm`, `diff/highlights.scm`, `go/highlights.scm`, `java/highlights.scm`, `python/highlights.scm`, `rust/highlights.scm`, `bash/highlights.scm`, `sql/highlights.scm`, and `terraform/highlights.scm` capturing `package` in Green, `import` in Orange, `sizeof` type arguments as `@type`, factory functions as Blue, records/guards as Green, annotations/decorators as Orange, method initializers as Blue, Rust attributes as Orange, Rust lifetimes as Green, Bash trap signals, positional parameters, redirection descriptors, case patterns, subscript wildcards, SQL null sentinels, table alias qualifiers, and relational objects, and Terraform block declarations and scope keywords in Green, schema blocks and types in Yellow, built-in functions in Blue, booleans in Magenta, and Base0 string interpolation delimiters and resource references matching bat).
